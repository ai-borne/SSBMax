...
import { Clock, CheckCircle2, ChevronRight, Send, AlertTriangle, Image as ImageIcon } from 'lucide-react';

interface TATSlideViewerProps {
  viewModel: PsychologyTestViewModel;
  userId: string;
  isOnline?: boolean;
}

export const TATSlideViewer: React.FC<TATSlideViewerProps> = ({
  viewModel,
  userId,
  isOnline = true
}) => {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between p-4 bg-slate-900 border border-slate-800 rounded-xl">
        <div>
          <h2 className="text-lg font-semibold text-slate-100">{strings.psychology.tatTitle}</h2>
      ...
      <div className="p-6 bg-slate-900 border border-slate-800 rounded-xl flex flex-col items-center justify-center min-h-[260px]">
        {currentSlide.content ? (
          <img
            src={currentSlide.content}
            alt="TAT Story Prompt"
            className="max-h-80 rounded-lg border border-slate-700 object-contain"
          />
        ) : (
          <div className="text-center p-8 text-slate-400">
            <ImageIcon className="w-12 h-12 mx-auto mb-2 opacity-50" />
            <p className="text-sm">Blank Slide (Final TAT Prompt)</p>
          </div>
        )}
      </div>
