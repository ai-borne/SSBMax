 ...
 * OIR (not OLQ-scored, no `SubmissionResultView`-compatible result doc) and GTO's 5 not-yet-ported
...
export interface NotificationResultTarget {
  submissionId: string;
  resultCollection: string;
}

/** Resolves a tapped notification to a `SubmissionResultView` target, or `null` if it isn't a graded-result notification with a supported testType (e.g. OIR, or the placeholder-only GTO sub-tests). */
export function resolveNotificationResultTarget(notification: SSBMaxNotification): NotificationResultTarget | null {
  ...
  return { submissionId, resultCollection };
}
