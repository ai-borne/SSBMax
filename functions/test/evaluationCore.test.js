...
test('Phase 1: runEvaluation rejects permission-denied when the submission belongs to another user', async () => {
  const db = makeFakeDb({
    [`submissions/${SUBMISSION_ID}`]: { userId: 'someone-else', analysisStatus: 'PENDING_ANALYSIS' }
  });
...
test('Phase 1: runEvaluation is an idempotent no-op when status is not PENDING_ANALYSIS', async () => {
  const db = makeFakeDb({
    [`submissions/${SUBMISSION_ID}`]: { userId: UID, analysisStatus: 'COMPLETED' }
  });
...
test('Phase 1: runEvaluation rejects failed-precondition when the user is already at their monthly quota', async () => {
  ...
  const db = makeFakeDb({
    [`submissions/${SUBMISSION_ID}`]: { userId: UID, analysisStatus: 'PENDING_ANALYSIS' },
...
test('Phase 1: runEvaluation happy path flips ANALYZING -> COMPLETED and writes the result doc', async () => {
  ...
  assert.equal(result.status, 'COMPLETED');
  assert.equal(db._store[`submissions/${SUBMISSION_ID}`].analysisStatus, 'COMPLETED');
  ...
  const analyzingWrite = db._writes.find((w) => w.path === `submissions/${SUBMISSION_ID}` && w.value.analysisStatus === 'ANALYZING');
...
test('Phase 1: runEvaluation flips to FAILED and writes no result doc when parseAndValidate never accepts', async () => {
  ...
  assert.equal(db._store[`submissions/${SUBMISSION_ID}`].analysisStatus, 'FAILED');
