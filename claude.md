...
**Tier-2 AI evaluation is now genuinely centralized** (per the Web SSB Test Flow Parity + Centralized Tier-2 Evaluation SSOT plan, complete through Phase 12): `functions/src/evaluation/core.js` is the one dispatcher — auth/ownership check, server-side quota re-check, retry, Gemini call, result write — that every `evaluate*` callable (`ppdtEvaluate.js`, `tatEvaluate.js`, `watEvaluate.js`, `srtEvaluate.js`, `sdEvaluate.js`, `gtoEvaluate.js`, `interviewEvaluate.js`) is a thin per-type wrapper over. Both KMP (via `EvaluationFunctionsClient`, `shared/.../domain/service/`) and web (via `SubmissionRepository.ts`) call the same functions — see `functions/CLAUDE.md`. **OIR is fully cut over**: `OIRTestScoreCalculator` no longer scores locally, it only formats server-computed `correctnessByQuestionId` from `evaluateOIRAnswers`. **WAT/SRT/SD/Interview/GTO/PPDT/TAT are "Shipped" but not yet cut over**: the new functions exist and both clients can call them, but KMP's legacy client-side orchestrators (`shared/.../analysis/*Orchestrator.kt`, `shared/.../ai/prompts/*.kt`, `KtorAIService.kt` et al.) and `functions/src/geminiProxy.js` are still the live default path, gated behind a feature flag, pending each type's bake-period Cutover (≥100 successful evaluations at <2% error rate, 3-day floor). **Do not delete those legacy files or `geminiProxy.js` as "unused"** — grep for their callers and check the plan's Cutover status per type before touching them; they are intentionally still live, not migration debt.
